...
// (FIX) Stage-transition discriminator. Set true inside the two Phase-C paths
// right before triggerStageTransition() runs; the legacy onBossDefeated() guard
// reads it to know whether the transition has already happened on the dying
// boss entity itself. Without this flag, the old `enemies.some(...'final')`
...
function startGame() {
    ...
    finalBossTransitionDone = false;  // (FIX) stage-transition discriminator reset on game restart
    midBossStreak = 0;
    bossClawCount = 1;
    bossClaws = [];
    octopusTentacles = [];
    ...
    if (bossActive) {
        ...
        while (bossClaws.length < bossClawCount) {
            bossClaws.push({
                ...
                // (FIX) drawBossClaw reads claw.y in createLinearGradient + chain
                // segments; it was never initialised here, so y was `undefined` →
                // NaN in createLinearGradient, throwing on every boss frame.
                y: bossEnemy ? bossEnemy.y : 100,
                length: 0,
        ...
        for (let ci = bossClaws.length - 1; ci >= 0; ci--) {
            ...
            if (claw.active) {
                ...
                // Claw follows boss x/y position loosely
                if (bossEnemy) {
                    claw.x += (bossEnemy.x + claw.offsetX - claw.x) * 0.1;
                    // (FIX) keep claw.y anchored to the boss so the gradient/strokes
                    // use finite numbers when boss y changes during entry/battle.
                    claw.y += (bossEnemy.y - claw.y) * 0.1;
                }
                // Check collision with player
        ...
        switch (enemy.type) {
            ...
            case 'final':
                ...
                if (enemy.dying) {
                    ...
                    // === Trigger hyperspace stage transition (single source of truth) ===
                    // Mark the "Phase C ran" flag BEFORE splicing the dying entity so
                    // onBossDefeated()'s guard reads the new state, not the entity.
...
// === STAGE TRANSITION (single source of truth) ===
// Called from BOTH Phase-C paths (the explicit body-off-screen Phase-C block,
// AND the Phase-B off-screen/time-cap splice that short-circuits to Phase C).
// Running it here guarantees finalBossTransitionDone is set before the dying
...
function triggerStageTransition() {
    ...
    // the real hyperspace state set by the rest of this function intact.
    finalBossTransitionDone = true;
    bossDefeated = true;
    player.lives = Math.min(5, player.lives + 1);
    bossClaws = [];
...
function onBossDefeated() {
    if (bossDefeated) {
        ...
        // off-screen / time-cap branch also splices the dying boss entity
        // WITHOUT running triggerStageTransition() (until this fix). So the old
        // guard absorbed lastDefeatedBossWasFinal and returned, and hyperspace
        // never started. Now we check the global flag finalBossTransitionDone
        // that both Phase-C paths set; only the path that actually ran the
        // transition gets to early-return. Otherwise the legacy path runs and
        // sets hyperspaceActive itself.
        if (lastDefeatedBossWasFinal && finalBossTransitionDone) {
            bossDefeated = false;
            bossActive = false;
            bossSpawned = false;
            lastDefeatedBossWasFinal = false;
            finalBossTransitionDone = false; // consumed
            return;
        }
